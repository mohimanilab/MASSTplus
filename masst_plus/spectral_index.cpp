...
void SpectralIndex::insertSpectrum(int id, std::string filename, int scan, int mass, double mass_raw, int charge, std::vector<Peak>& peaks, std::ofstream& output) {
	...
	for (int i = 0; i < peaks.size(); i++) {
		//std::cout << "The peak " << peaks[i].pos << "\t" << peaks[i].mag_normalized << " has unshifted entry " << peaks[i].pos_rounded << std::endl;
		peak_count++;
		...
		//std::cout << "There're " << peaks.size() << " number of peaks in total " << std::endl;
		//std::cout << "The peak " << peaks[i].pos << "\t" << peaks[i].mag_normalized << " has shifted entry " << std::round((mass_raw * charge - peaks[i].pos) / pos_bin_size) <<" The shifted difference is " << mass_raw * charge - peaks[i].pos << std::endl;
	}
}
...
void SpectralIndex::findPotentialMatches(std::string entries_str, double query_precursor, int query_peak_index, double query_peak, double query_magnitude, double precursor_tolerance, double peak_tolerance, std::unordered_map<std::string, std::vector<Match>>& peak_matches, std::unordered_map<std::string, double>& precursors_list) {
	...
	while (!done) {
		...
		if ((precursor_tolerance < 0 || std::abs(db_precursor - query_precursor) <= precursor_tolerance) && std::abs(query_peak - entry.peak_pos) <= peak_tolerance) {
			...
			//std::cout << "query peak " << query_peak << " " << query_magnitude << " matching library spectra " << entry.meta << " at peak " << entry.peak_pos << " " << entry.magnitude << ", dot product " << entry.magnitude * query_magnitude << std::endl;
			//std::cout << "Match with lib entry " << entry.meta << " query_index=" << match.a << " lib_index=" << match.b << " query magnitude:" << query_magnitude << " lib magnitude:" << entry.magnitude  << std::endl;
		}
	}
}
