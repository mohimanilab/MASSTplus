...
void print_help_message(){
  ...
  printf("       -q <precursor-tolerance>: precursor mass tolerance, default 0.025\n");
	return;
}
...
// find maximum dot product value
double getMaxDotProduct(std::vector<Match>& matches) {
	double max_dot_product = 0;
	for (int i = 0; i < matches.size(); i++) {
		max_dot_product += matches[i].val;
	}
...
void findMatches(FILE* outfile, double threshold, SpectraMGF& spectrum, int num_peaks, std::unordered_map<std::string, std::vector<Match>>& peak_matches, std::unordered_map<std::string, std::vector<Match>>& peak_matches_shifted, std::unordered_map<std::string, double>& db_precursors, SpectralIndex& spectral_index, bool hybrid_matching) {
	for(auto& [entry_key, precursor]: db_precursors) {
	  	...
	  	double max_dot_product = getMaxDotProduct(peak_matches[entry_key]);
		double max_dot_product_shifted;
		if (!hybrid_matching) {
			max_dot_product_shifted = getMaxDotProduct(peak_matches_shifted[entry_key]);
		}

		if (peak_matches[entry_key].size() > 0 && max_dot_product > threshold) {
			getTopMatches(peak_matches[entry_key], top_matches, num_peaks);
		}

		if (!hybrid_matching && peak_matches_shifted[entry_key].size() > 0 && max_dot_product_shifted > threshold) {
...
int main(int argc, char** argv){
  ...
  std::string lib_path = "library";
	std::string output_file_name = "matches-all.tsv";
	bool query_is_list = false;
	double threshold=0.7;
  bool error_tolerant = false;
	bool hybrid_matching = true;
	double precursor_tolerance = 0.025;
  double peak_tolerance = 0.02;
	bool debug = true;
